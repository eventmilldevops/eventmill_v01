...
lineage and never refuses a map. **N3 is being built in slices**: N3a (the
flow-map join and the five completeness grades) and N3b (controls) landed the
same day (`2026-09-19-n3a-n3b-join-grades-controls.md`). Next: N3c mitigation
focus and coverage, N3d taxonomy reconciliation. Neither needs a provider. N5,
the projector's own `normalize_flow_map`, can run in parallel.
