...
**Status:** built, scripted only. Suite **1705 → 1736**. **No live model call
has been made yet.**
...
- 31 new tests; full suite 1736; 36 schemas valid; manifest validation
...
- **Not run against a provider.** Every test uses a scripted reply. The next
  step is one live run on `gcp_gemini` against the 9-node pair.
